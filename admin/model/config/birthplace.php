...
class ModelConfigBirthplace extends Model {
	...
	public function deleteBirthplace($birthplace_id) {
		$sql = "UPDATE " . DB_PREFIX . "birthplace SET deleted = 1 WHERE birthplace_id = '" . (int)$birthplace_id . "'";
		$this->db->query($sql);
	}

	public function getBirthplace($birthplace_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "birthplace WHERE birthplace_id = $birthplace_id AND deleted = 0");
...
// print($sql); exit;
	...
	public function getTotalBirthplaces() {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "birthplace WHERE deleted = 0");
