...
*/
class vmAbstractLanguage1 {
/** @var boolean If true, highlights string not found */
	var $_debug = false;
	var $modules = array();
	var $key_varname = '';
	
	function vmAbstractLanguage1() {
		$this->setDebug();
	}
	...
	*/
	function setCharset($module,$charset='') {
		if( !empty( $charset )) {
			...
			$this->modules[$module]['CHARSET'] = vmGetCharset1();
		}
	}
	/**
	...
	*/
	...
	function safe_utf8_encode( $text, $charset ) {
		if( strtolower($charset) == 'utf-8' &&  !vmAbstractLanguage1::seems_utf8( $text )) {
			// safely decode and reencode the string
			$text = utf8_encode($text);
		}
		// This converts the currency symbol from HTML entity to the utf-8 symbol
		// example:  &euro; => €
		$text = vmHtmlEntityDecode1( $text, null, vmGetCharset1() );
...
class mosAbstractLanguage1 extends vmAbstractLanguage1 { }
class vmLanguage1 extends vmAbstractLanguage1 { }
class phpShopLanguage1 extends vmLanguage1 { }
