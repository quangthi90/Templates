 ...
 * @return boolean
 */
function process_date_time1(&$d,$field,$type="") {
 ...
 */
function vmValidateEmail1( $email ) {
 ...
 * @return		boolean
 */
function vmValidateName1( $string ) {
	/*
 ...
 * @return boolean The result of the validation
 */
function vmValidateEUVat1( $euvat ){
 ...
 */
function read_file1( $file, $defaultfile='' ) {
 ...
 */
function include_class1($module) {
 ...
 */
function editorScript1($editor1='', $editor2='') {
	?>
...
*/
function vmCreateMail1( $from='', $fromname='', $subject='', $body='' ) {
	...
	if( $subject ) {
		$mail->Subject 	= vmAbstractLanguage1::safe_utf8_encode( $subject, $mail->CharSet );
	}
	if( $body) {
		$mail->Body 	= $body;
	}
...
*/
function vmMail1($from, $fromname, $recipient, $subject, $body, $Altbody='', $mode=false, $cc=NULL, $bcc=NULL, $images=null, $attachment=null ) {
	global $mosConfig_debug;

		// Filter from, fromname and subject
	if (!vmValidateEmail1( $from ) || !vmValidateName1( $fromname ) || !vmValidateName1( $subject )) {
		return false;
	}
	
	$mail = vmCreateMail1( $from, $fromname, $subject, $body );
	
	if( $Altbody != "" ) {
		// In this section we take care for utf-8 encoded mails
		$mail->AltBody = vmAbstractLanguage1::safe_utf8_encode( $Altbody, $mail->CharSet );
	}
	...
	if( $mail->ContentType == "text/plain" ) {
		$mail->Body = vmAbstractLanguage1::safe_utf8_encode( $mail->Body, $mail->CharSet );
	}
	if( is_array($recipient) ) {
		foreach ($recipient as $to) {
			if( vmValidateEmail1( $to )) {
				$mail->AddAddress($to);
			}
		}
	} else {
		if( vmValidateEmail1( $recipient )) {
 ...
 */
function vmHtmlEntityDecode1($string, $quote_style = ENT_COMPAT, $charset = null) {
 ...
 * @since 1.1.0
 */
function vmGetUnEscaped1( $string ) {
 ...
 * @return mixed
 */
function vmReadFileChunked1($filename,$retbytes=true) {
 ...
 * @since 1.1.0
 */
function vmSafeFileName1( $filename ) {
