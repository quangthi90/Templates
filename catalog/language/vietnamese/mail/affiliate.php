<?php
// Text
$_['text_subject']  = '%s - Chương Trình Đại lý';
$_['text_welcome']  = 'Cám ơn bạn đã tham gia chương trình Đại lý của %s!';
$_['text_login']    = 'Tài khoản của bạn đã được khởi tạo, bạn có thể sử dụng Email và mật khẩu để đăng nhập bằng cách truy cập Website của chúng tôi theo đường dẫn:';
$_['text_approval'] = 'Bạn có thể đăng nhập vào tài khoản Đại lý bằng email và mật khẩu theo đường dẫn sau:';
$_['text_services'] = 'Khi đăng nhập bạn có thể xem mã Đại lý, tạo các link giới thiệu của mình và theo dõi tiền Hoa hồng thông qua đường dẫn:';
$_['text_thanks']   = 'Trân trọng cám ơn! Chúc Bạn phát triển được nhiều Đại lý khác & luôn đạt doanh số cao nhất!';
$_['text_new_affiliate']        = 'Đại lý mới';
$_['text_signup']		        = 'Một đại lý mới đã đăng ký:';
$_['text_store']		        = 'Cửa hàng:';
$_['text_firstname']	        = 'Họ:';
$_['text_lastname']		        = 'Tên:';
$_['text_company']		        = 'Công ty:';
$_['text_email']		        = 'E-Mail:';
$_['text_telephone']	        = 'Điện thoại:';
$_['text_website']		        = 'Website:';
$_['text_order_id']             = 'STT:';
$_['text_transaction_subject']  = '%s - Hoa hồng đại lý';
$_['text_transaction_received'] = 'Bạn đã nhận được %s hoa hồng!';
$_['text_transaction_total']    = 'Tổng số hoa hồng bạn nhận được lúc này là %s.';
?>